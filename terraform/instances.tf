...
# front-proxy
resource "digitalocean_droplet" "do-front-proxy-instance" {
  image              = var.do_image
  name               = format("do-front-proxy-instance-%s", var.do_region)
  region             = var.do_region
  size               = var.do_size
  ssh_keys           = var.do_ssh_keys
  private_networking = true
  ...
  tags = [
    format("do-front-proxy-instance-%s", var.do_region)
  ]
}
